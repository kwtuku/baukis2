...
feature "職員によるアカウント管理" do
  ...
  before do
    switch_namespace(:staff_member)
    login_as_staff_member(staff_member)
    click_link "アカウント"
    click_link "アカウント情報編集"
  end

  scenario "顧客がメールアドレス、⽒名、フリガナを更新する" do
    fill_in "メールアドレス", with: "test@oiax.jp"
    fill_in "staff_member_family_name", with: "試験"
    fill_in "staff_member_given_name", with: "花⼦"
